...
Computes solve-rate upper bound performance. A problem is solved if ANY candidate
(before or after repair) passes all tests.
Validates oracle >= repair pass@1 (upper bound invariant).
...
class OracleAnalysisAgent(BaseAgent):
    """Computes oracle solve-rate upper bound with invariant checking."""
    ...
    def analyze(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Compute oracle solve-rate with upper-bound validation.
        
        Input: {"problems": [{task_id, dataset, any_candidate_passed}],
